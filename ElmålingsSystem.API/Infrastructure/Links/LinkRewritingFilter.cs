...
namespace ElmålingsSystem.API.Infrastructure
{
    ...
    {
        //lets us create IUrlHelpers on the fly
        ...
        {
            //context.Result = the model that's being returned from some controller, which will be casted as ObjectResult
            var asObjectResult = context.Result as ObjectResult;

            bool shouldSkip = asObjectResult?.StatusCode >= 400 //(it's an error, and won't have any links on it)
                || asObjectResult?.Value == null    //if the value of the response is null
                || asObjectResult?.Value as Resource == null;   //(if it isn't null, but if we try to cast it as Resource, that's null)

            if (shouldSkip) return next();

            //else it's an actual response, and we'll look for any Link objects on the response
        ...
        private static void RewriteAllLinks(object model, LinkRewriter rewriter)
        {
            ...
            //grab all the properties from model using reflection(GetTypeInfo())
            ...
            //property, that is type of our Link class
            var linkProperties = allProperties
                .Where(p => p.CanWrite && p.PropertyType == typeof(Link));

            foreach (var linkProperty in linkProperties)
            {
                var rewritten = rewriter.Rewrite(linkProperty.GetValue(model) as Link); //rewrite the current selected linkProperty (casted as Link)
                if (rewritten == null) continue;

                linkProperty.SetValue(model, rewritten);    //replaces the original link, with this new link(CanWrite?)
        ...
        private static void RewriteLinksInNestedObjects(IEnumerable<PropertyInfo> objectProperties, object model, LinkRewriter rewriter)
        {
            ...
            {
                if (objectProperty == typeof(string) || objectProperty.PropertyType == typeof(char))
                {
                    continue;
                }
